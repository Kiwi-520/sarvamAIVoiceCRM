...
class CallAnalytics:
    ...
    def process_audio_files(self, audio_paths: List[str]) -> Dict[str, str]:
        ...
        try:
            # FIXED: Changed 'client' to 'self.client'
    ...
    def analyze_transcription(self, conversation_path: str, output_dir: Path, file_name: str) -> Dict:
        try:
            ...
            analysis_prompt = textwrap.dedent(self.ANALYSIS_PROMPT_TEMPLATE.format(transcription=transcription))
            messages = [
            ...
            response = self.client.chat.completions(model="sarvam-30b", messages=messages)
        ...
        try:
            with open(summary_path, "w", encoding="utf-8") as f:
                ...
                for file_name, data in self.transcriptions.items():
                    ...
                    summary_prompt = textwrap.dedent(self.SUMMARY_PROMPT_TEMPLATE.format(analysis_text=analysis_text))
                    messages = [
...
st.title("🎙️ AI Call Analytics Dashboard")
st.markdown("Upload a customer support call to automatically generate speaker-wise transcripts, sentiment analysis, and a structured summary.")

# Failsafe if the API key isn't loaded properly
if not api_key:
...
if uploaded_file is not None:
    if st.button("Run Analysis"):
        with st.spinner("Processing audio through the engine... This takes about a minute."):
            ...
            try:
                ...
                transcriptions = analytics.process_audio_files([tmp_audio_path])
                ...
                outputs_dir = "outputs"
                summary_files = [f for f in os.listdir(outputs_dir) if f.startswith("summary_")]
                if summary_files:
                    latest_summary = sorted(summary_files)[-1]
                    with open(os.path.join(outputs_dir, latest_summary), "r", encoding="utf-8") as f:
                        st.text_area("Summary Report", f.read(), height=300)
